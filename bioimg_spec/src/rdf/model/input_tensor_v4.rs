...
pub enum InputTensorV4ParsingError{
...
impl TryFrom<InputTensorMetadataMsg> for InputTensorMetadata{
    type Error = InputTensorV4ParsingError;
    fn try_from(message: InputTensorMetadataMsg) -> Result<Self, Self::Error> {

        fn ensure_axis_exists(message: &InputTensorMetadataMsg, preproc_axis_id: &AxisId) -> Result<(), InputTensorV4ParsingError>{
            message.axes.iter()
                .map(|ax| ax.id())
                .find(|ax_id| {
                    ax_id == preproc_axis_id
                })
                .ok_or(InputTensorV4ParsingError::PreprocessingReferencesNonExistingAxis)
                .map(|_| ())
        }
